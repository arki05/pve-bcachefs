...
# Load errors of Custom/ plugins are only warned about, so grep for them.
if perl -e 'use PVE::Storage;' 2>&1 | grep -i 'BcachefsPlugin'; then
    echo "plugin failed to load, removing" >&2
